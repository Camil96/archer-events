...
import { Users, Shield, Search, Check, X } from 'lucide-react';
import toast from 'react-hot-toast';
import { User as UserType, UserRole, Brand } from '@/types';
...
const TeamSection: React.FC<TeamSectionProps> = ({ user: currentUser }) => {
  ...
  const updateUserBrandAccess = async (userId: string, brandAccess: string[]) => {
    ...
    try {
      ...
      toast.success('Merkt toegang bijgewerkt.');
    } catch (error) {
      console.error('Error updating user brand access:', error);
      toast.error('Kon merkt toegang niet bijwerken.');
    } finally {
      setIsUpdating(null);
    }
  };
  ...
  const getBrandName = (brandKey: string) =>
    brandOptions.find((b) => b.brand === brandKey)?.brand_name || brandKey;
  ...
  const filteredUsers = users.filter((u) => {
    ...
    const matchesBrand =
      brandFilter === 'all' || (u.brand_access && u.brand_access.includes(brandFilter));
            ...
            <tbody className="bg-white divide-y divide-neutral-200">
              {filteredUsers.map((userItem) => (
                <tr key={userItem.id} className="hover:bg-neutral-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getRoleBadgeColor(userItem.role)}`}
                    >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex flex-wrap gap-1">
                      {(userItem.brand_access?.length ?? 0) > 0 ? (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center justify-end space-x-2 flex-wrap">
                      <select
                        value={userItem.role}
                        onChange={(e) =>
                          updateUserRole(userItem.id, e.target.value as UserRole)
                        }
                        disabled={
                          isUpdating === userItem.id || userItem.id === currentUser.id
                        }
                      ...
                      <select
                        ...
                        disabled={isUpdating === userItem.id}
                        className="text-xs border border-neutral-300 rounded px-2 py-1 focus:ring-1 focus:ring-archer-blue min-w-[100px]"
                        size={1}
                      >
                        {brandOptions.map((b) => (
                          <option key={b.brand} value={b.brand}>
                      ...
                      {isUpdating === userItem.id && (
                        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-archer-blue" />
                      )}
                    </div>
                  </td>
                </tr>
